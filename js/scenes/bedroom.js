(function(){
 const CLOUD_TIMING={initialGapMin:9000,initialGapMax:27000,spawnGapMin:50000,spawnGapMax:125000,maxVisible:2,speed:{large:.72,medium:.92,small:1.12},yMin:18,yMax:225};
 ...
 function spawnCloud(size){if(!active||!sizes[size]||clouds.length>=CLOUD_TIMING.maxVisible)return false;const x=266,y=rand(CLOUD_TIMING.yMin,CLOUD_TIMING.yMax);clouds.push({size,x,y});return true;}
 function scheduleCloud(first=false){clearTimeout(cloudTimer);if(!active)return;const delay=first?rand(CLOUD_TIMING.initialGapMin,CLOUD_TIMING.initialGapMax):rand(CLOUD_TIMING.spawnGapMin,CLOUD_TIMING.spawnGapMax);cloudTimer=setTimeout(()=>{if(active&&clouds.length<CLOUD_TIMING.maxVisible){const list=['large','medium','small'];spawnCloud(list[Math.floor(Math.random()*list.length)])}scheduleCloud();},delay);}
 ...
 function enter(){active=true;forced=false;scene.setAttribute('aria-hidden','false');document.body.classList.add('bedroom-mode');if(window.RADevState){window.RADevState.scene='bedroom';window.RADevState.bedroomRichState='lounge_idle'}setRichState('lounge_idle');scheduleAmbient();scheduleCloud(true);lastFrame=0;raf=requestAnimationFrame(animate);}

(function(){
 ...
 function finish(){const trip=current();if(!trip)return false;const completedAt=new Date().toISOString(),result=update({status:'completed',currentActivity:null,completedAt});const life=window.RAState.get().life.desires;const completed=[...(life.completed||[])];if(!completed.some(x=>x.id===trip.id))completed.push({id:trip.id,title:trip.title,destination:trip.destination,purpose:trip.purpose,completedAt});window.RAState.patch('life.desires.completed',completed);window.RALife?.unlockApp?.('receipts');window.RALife?.receipt?.({id:`desire:${trip.id}`,caption:'butter chicken under the stars.',env:'curb',lane:'desire'});window.RALife?.remember?.({id:`desire:${trip.id}`,text:'butter chicken under the stars',lane:'desire'});window.RAState.recordEvent({id:`desire-completed:${trip.id}`,type:'desire_completed',tripId:trip.id,title:trip.title,destination:trip.destination,purpose:trip.purpose});return result}

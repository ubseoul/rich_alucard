(function(){
 ...
 function placeCharacter(img,anchorX,scale=characterScale){if(!img||window.RAPresentationDirector?.current?.())return;const sx=payoffCanvas?.getBoundingClientRect().width/(payoffCanvas?.width||270)||1,sy=payoffCanvas?.getBoundingClientRect().height/(payoffCanvas?.height||480)||1;img.style.width=`${80*scale*sx}px`;img.style.height=`${96*scale*sy}px`;img.style.left=`${Math.round((anchorX-40*scale)*sx)}px`;img.style.top=`${Math.round((350-88*scale)*sy)}px`;img.classList.add('visible')}
 // Presentation Director: story scenes (arrival/meet/aftermath) and the Supra payoff are framed by the Director.
 const storyDirected=()=>!!window.RAPresentationDirector&&!window.__pdLegacy;
 function layoutDockActors(){if(storyDirected()&&RAPresentationDirector.current()?.stage==='jdm-imports-docks'&&RAScenes.current()!=='jdmCombat'){for(const img of Object.values(dockActors))img?.classList.add('visible');RAPresentationDirector.relayout();return}for(const [slot,img] of Object.entries(dockActors)){if(img){window.RAStageLayout.layout('jdm-imports-docks',slot,dockCanvas,img,characterScale);img.classList.add('visible')}}}
 function placePayoffObjects(){if(window.RAPresentationDirector?.current?.())return;if(payoffCar){window.RAStageLayout.layout('jdm-imports-docks','supra',payoffCanvas,payoffCar);payoffCar.classList.add('visible')}const sx=payoffCanvas?.getBoundingClientRect().width/(payoffCanvas?.width||270)||1,sy=payoffCanvas?.getBoundingClientRect().height/(payoffCanvas?.height||480)||1;if(payoffKey){payoffKey.style.left=`${67*sx}px`;payoffKey.style.top=`${319*sy}px`;payoffKey.style.width=`${24*sx}px`;payoffKey.style.height=`${16*sy}px`;payoffKey.classList.add('visible')}}
 ...
 function sceneEntry(id,scope){document.body.classList.add('jdm-mode');document.body.classList.remove('jdm-battle');show(dock,id!=='supraPayoff');show(payoff,id==='supraPayoff');
  const stage=window.RAStages.get('jdm-imports-docks');
  if((id==='jdmDock'||id==='jdmAftermath')&&storyDirected()){dockCanvas.dataset.asset=ENVIRONMENT;dockStage=window.RAPresentationDirector.enter({stage:'jdm-imports-docks',mode:'dialogue',beat:'story',scope,host:dock,env:dockCanvas,envAsset:ENVIRONMENT,actors:dockActors})}
  else if(id==='jdmDock'||id==='jdmAftermath')dockStage=window.RAStageLayout.activate('jdm-imports-docks',dockCanvas,dockActors,scope,()=>characterScale);
  if(id==='supraPayoff'&&storyDirected()){payoffCanvas.dataset.asset=ENVIRONMENT;for(const el of [payoffRich,payoffCar,payoffKey])el?.classList.add('visible');window.RAPresentationDirector.enter({stage:'jdm-imports-docks',mode:'cinematic',beat:'payoff',scope,host:payoff,env:payoffCanvas,envAsset:ENVIRONMENT,actors:{rich:payoffRich,supra:payoffCar},worldLayers:payoffKey?[{el:payoffKey,rect:stage.director.keyProp}]:[]})}
  ...
  if(id==='supraPayoff'){paintEnvironment(payoffCanvas,payoffEnvironment);if(!storyDirected()){placeCharacter(payoffRich,55);placePayoffObjects()}renderPayoff()}
 }
 function sceneExit(id){if(['jdmDock','jdmAftermath','supraPayoff'].includes(id))window.RAPresentationDirector?.exit();if(id==='supraPayoff'){payoffRich?.classList.remove('visible');payoffCar?.classList.remove('visible');payoffKey?.classList.remove('visible')}if(!['jdmDock','jdmAftermath','supraPayoff'].includes(id))show(dock,false)}

(function(){
  ...
  function refresh(){
    ...
    const lines=[`SCENE: ${scene}`,`RICH: ${richState}`,...(scene==='bedroom'?[`CLOUDS: ${window.RABedroom?.cloudCount?.()??0}`]:[]),`CASH: $${life.resources?.money??0}`,`LOCATION: ${life.world?.location??'--'}`,`CLOUT: ${life.resources?.clout??'--'}`,`PHONE LEARNED: ${!!life.phone?.learned}`,`ACTIVE TRIP: ${tripLine}`,`CEO DEFEATED: ${!!save.encounters?.ceo_prince?.defeated}`,`ASSISTANT: ${JSON.stringify(save.characters?.ceo_assistant_001||{})}`,`AUDIO: ${audio?audio.currentTime.toFixed(2):'--'}`,`REVENGE STORED: ${s.revengeStoredDamage??0}`,`FONT: ${document.documentElement.dataset.devFont||'control'}`,`TINT: ${document.querySelector('#screen')?.classList.contains('cartridge-tint')?'ON':'OFF'}`];
